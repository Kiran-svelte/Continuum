...
const PORTAL_ACCENTS: Record<string, React.CSSProperties> = {
  blue: {
    '--primary': '#2563eb',
    '--accent-primary': '#2563eb',
    '--accent-primary-hover': '#1d4ed8',
    '--accent-soft': 'color-mix(in srgb, #2563eb 12%, transparent)',
  } as React.CSSProperties,
  emerald: {
    '--primary': '#059669',
    '--accent-primary': '#059669',
    '--accent-primary-hover': '#047857',
    '--accent-soft': 'color-mix(in srgb, #059669 12%, transparent)',
  } as React.CSSProperties,
  amber: {
    '--primary': '#d97706',
    '--accent-primary': '#d97706',
    '--accent-primary-hover': '#b45309',
    '--accent-soft': 'color-mix(in srgb, #d97706 13%, transparent)',
  } as React.CSSProperties,
  violet: {
    '--primary': '#7c3aed',
    '--accent-primary': '#7c3aed',
    '--accent-primary-hover': '#6d28d9',
    '--accent-soft': 'color-mix(in srgb, #7c3aed 12%, transparent)',
  } as React.CSSProperties,
};
...
export function PortalLayout({ children, config }: { children: React.ReactNode, config: PortalConfig }) {
  ...
  const portalAccent = PORTAL_ACCENTS[config.accentColor || 'blue'] ?? PORTAL_ACCENTS.blue;
  ...
  return (
    <div
      data-portal={config.portalSlug}
      style={portalAccent}
      className={`relative flex h-dvh overflow-hidden bg-[var(--background)] text-[var(--foreground)] selection:bg-[var(--primary)] selection:text-[var(--primary-foreground)] ${isNavigating ? 'cursor-progress' : ''}`}
    >
      ...
      <aside
        className={`
        pulse-sidebar fixed inset-y-0 left-0 z-50 flex w-[var(--sidebar-width)] flex-col border-r border-[var(--border)]
        shadow-[var(--shadow-md)] transition-transform duration-300 ease-[var(--motion-enter)]
      ...
      >
          ...
          {config.navItems.map((item, idx) => {
            ...
            const linkEl = (
              <Link
                ...
                className={`
                  group relative flex w-full items-center gap-3 rounded-[var(--radius)] px-3 py-2.5 text-sm font-medium transition-[background-color,color,box-shadow] duration-150
                  ${isActive
            ...
            return (
              <React.Fragment key={`${item.href}-${idx}`}>
                {isNewGroup && (
                  <div
                    className={`${idx > 0 ? 'mt-4 border-t border-[var(--border)] pt-3' : 'pt-1'} px-2 pb-1 text-[11px] font-bold uppercase tracking-widest text-[var(--muted-foreground)] opacity-70`}
                  >
                    {item.group}
                  </div>
                )}
        ...
        <div
          className="mt-auto flex shrink-0 items-center gap-3 border-t border-[var(--border)] p-4"
          title={`${displayName}${displayEmail ? ` - ${displayEmail}` : ''}`}
        >
      ...
      <main className="relative z-10 flex h-full min-w-0 flex-1 flex-col overflow-hidden">
        <header className="sticky top-0 z-30 flex min-h-[var(--topbar-height)] shrink-0 items-center justify-between gap-3 border-b border-[var(--border)] bg-[color-mix(in_srgb,var(--background)_90%,transparent)] px-4 py-3 backdrop-blur-md lg:px-8">
        ...
        <div className="relative min-h-0 w-full flex-1 overflow-y-auto page-enter">{children}</div>
      </main>
    </div>
  );
}
