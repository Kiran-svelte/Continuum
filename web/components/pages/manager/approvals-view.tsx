    ...
    <div className="p-4 sm:p-6 pb-32">
      <div className="space-y-8">
        ...
        <div className="space-y-2">
          ...
          <Skeleton className="h-4 w-64 bg-[var(--muted)]" />
        </div>
        ...
        <GlassPanel className="p-4 flex flex-col sm:flex-row gap-3">
          <Skeleton className="h-10 flex-1 bg-[var(--muted)] rounded-lg" />
          <Skeleton className="h-10 w-40 bg-[var(--muted)] rounded-lg" />
          <Skeleton className="h-10 w-10 bg-[var(--muted)] rounded-lg" />
        </GlassPanel>

        {/* Bulk actions skeleton */}
        <div className="h-12 flex items-center justify-between">
          <Skeleton className="h-6 w-32 bg-[var(--muted)] rounded-md" />
        ...
        <div className="space-y-4">
          {[...Array(3)].map((_, i) => (
            <GlassPanel key={i} className="p-4">
              ...
              <div className="mt-4 h-px bg-[var(--muted)]" />
                ...
                <GlassPanel className="p-4 flex flex-col sm:flex-row gap-3">
                  <div className="relative flex-1">
                    ...
                    <Input
                      ...
                      className="w-full h-10 pl-10 pr-4 bg-[var(--muted)] border border-[var(--border)] rounded-lg text-foreground placeholder:text-muted-foreground focus:ring-2 focus:ring-primary/50 focus:outline-none"
                    />
                  </div>
                  <div className="relative">
                    ...
                    <Select
                      ...
                      aria-label="Filter requests by leave type"
                      className="appearance-none w-full sm:w-48 h-10 pl-10 pr-8 bg-[var(--muted)] border border-[var(--border)] rounded-lg text-foreground focus:ring-2 focus:ring-primary/50 focus:outline-none"
                    >
                  ...
                  <Button variant="ghost" onClick={() => { setFilterName(''); setFilterLeaveType('all'); }} className="h-10 text-muted-foreground hover:text-foreground hover:bg-[var(--muted)]">
                    Clear
                  </Button>
                </GlassPanel>
              </FadeIn>
              ...
              {loading && requests.length === 0 ? (
                ...
                <FadeIn>
                  <GlassPanel className="text-center py-24">
                    <Inbox className="w-16 h-16 text-muted-foreground mx-auto mb-6" />
                    <h3 className="text-xl font-semibold text-foreground mb-2">No matching requests</h3>
            ...
            >
              {historyLoading && historyRequests.length === 0 ? (
                ...
                <GlassPanel className="text-center py-24 border-red-500/30">
                  ...
                  <h3 className="text-xl font-semibold text-foreground mb-2">Error loading history</h3>
                  ...
                  {historyPagination && historyPagination.pages > 1 && (
                    <div className="flex justify-center items-center gap-4 pt-4">
                      <Button
                        ...
                        className="bg-[var(--muted)] border-[var(--border)] text-muted-foreground hover:bg-[var(--muted)] hover:text-foreground"
                      >
      ...
      <AnimatePresence>
        {commentingId && (
          <Modal
            ...
            className="glass-panel border-[var(--border)] text-foreground"
          >
            <Textarea
              value={comment}
              onChange={(e) => setComment(e.target.value)}
              placeholder="e.g., 'Enjoy your time off!'"
              className="w-full mt-4 p-3 bg-[var(--muted)] border border-[var(--border)] rounded-lg text-foreground placeholder:text-muted-foreground focus:ring-2 focus:ring-primary/50 focus:outline-none"
              rows={3}
            />
            <ModalFooter className="mt-6">
              <Button variant="ghost" onClick={cancelAction} className="text-muted-foreground hover:text-foreground hover:bg-[var(--muted)]">Cancel</Button>
              <Button
      ...
      <AnimatePresence>
        {bulkAction && (
          ...
          >
            {!bulkProcessing && bulkErrors.length === 0 && (
              <>
                <Textarea
                  ...
                  rows={3}
                />
                <p className="text-xs text-muted-foreground mt-2">This comment will be applied to all {selectedCount} requests.</p>
              </>
            )}
            ...
            {(bulkProcessing || bulkErrors.length > 0) && (
              <div className="mt-4 space-y-4">
                ...
                {bulkErrors.length > 0 && !bulkProcessing && (
                  <div className="mt-4 space-y-3 max-h-40 overflow-y-auto p-3 bg-[var(--muted)] rounded-lg border border-red-500/30">
            ...
            <ModalFooter className="mt-6">
              <Button variant="ghost" onClick={closeBulkDialog} disabled={bulkProcessing} className="text-muted-foreground hover:text-foreground hover:bg-[var(--muted)]">
    ...
    <TiltCard>
      <GlassPanel className={isSelected ? 'border-primary/50 shadow-[0_0_20px_rgba(var(--primary-rgb),0.2)]' : ''}>
        <div className="p-4 relative z-10">
          <div className="flex items-start gap-4">
            <Button onClick={() => onToggleSelect(req.id)} className="mt-1">
              {isSelected ? <CheckSquare className="w-6 h-6 text-primary" /> : <Square className="w-6 h-6 text-muted-foreground hover:text-foreground" />}
            </Button>
            <div className="flex-1 min-w-0">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-base font-bold text-foreground">{req.employee.first_name} {req.employee.last_name}</p>
                    <p className="text-xs text-muted-foreground">{req.employee.department || 'No Department'}</p>
                  </div>
                </div>
                <p className="text-xs text-muted-foreground mt-2 sm:mt-0">{timeAgo(req.created_at)}</p>
              </div>
              <div className="mt-4 pl-1 space-y-1 text-sm">
                <p><strong className="text-muted-foreground font-medium">Type:</strong> {req.leave_type}</p>
                <p><strong className="text-muted-foreground font-medium">Dates:</strong> {formatDate(req.start_date)} - {formatDate(req.end_date)} ({req.total_days} days)</p>
                {req.reason && <p><strong className="text-muted-foreground font-medium">Reason:</strong> <span className="text-foreground/90">{req.reason}</span></p>}
              </div>
            </div>
          </div>
        </div>
        ...
        {req.constraint_result && (
          <div className="border-t border-[var(--border)] px-5 py-4 bg-[var(--muted)] space-y-3 relative z-10">
            <h4 className="text-sm font-semibold text-foreground/80 flex items-center gap-2"><Bot className="w-5 h-5 text-primary" /> Constraint Analysis</h4>
          </div>
        )}
        <div className="border-t border-[var(--border)] p-4 flex justify-end gap-3 bg-[var(--muted)] rounded-b-2xl relative z-10">
...
function HistoryCard({ req }: { req: LeaveRequest }) {
  ...
  } as Record<string, { icon: typeof CheckCircle; color: string; bg: string }>)[req.status] || { icon: History, color: 'text-muted-foreground', bg: 'bg-[var(--muted)]' };
    ...
    <TiltCard>
      <GlassPanel className="p-4">
        <div className="flex items-start gap-4 relative z-10">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex justify-between">
              <p className="font-bold text-foreground">{req.employee.first_name} {req.employee.last_name}</p>
              <Badge variant={statusBadgeVariant(req.status)}>{req.status}</Badge>
            </div>
            <p className="text-xs text-muted-foreground">{req.leave_type} · {req.total_days} day{req.total_days !== 1 ? 's' : ''}</p>
            <p className="text-sm text-foreground/80 mt-2">{formatFullDate(req.start_date)} - {formatFullDate(req.end_date)}</p>
            {req.approved_at && <p className="text-xs text-muted-foreground mt-1">Processed on {formatFullDate(req.approved_at)} by {req.approver?.first_name}</p>}
            {req.approver_comments && <p className="text-xs italic text-muted-foreground mt-1">&quot;{req.approver_comments}&quot;</p>}
          </div>
        </div>
      </GlassPanel>
    </TiltCard>
