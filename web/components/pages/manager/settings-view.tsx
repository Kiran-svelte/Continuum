...
import { jwtGetUser, jwtSendPasswordResetEmail } from '@/lib/auth-client';
...
function AnimatedToggleSwitch({
  ...
  return (
    <Button
      ...
      className={`relative w-14 h-8 rounded-full transition-colors duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--accent)] ${
        disabled ? 'opacity-50 cursor-not-allowed' : ''
      } ${value ? 'bg-primary' : 'bg-[var(--accent)]'}`}
    >
      <motion.div
        className="absolute top-1 left-1 w-6 h-6 rounded-full bg-[var(--card)] shadow-md"
        layout
        transition={spring}
        style={{
    ...
    <div className="p-4 sm:p-6 pb-32">
      <div className="space-y-8">
        ...
        <div className="space-y-2">
          <Skeleton className="h-8 w-56 bg-[var(--muted)]" />
          <Skeleton className="h-4 w-72 bg-[var(--muted)]" />
        </div>
          ...
          {[...Array(4)].map((_, i) => (
            <GlassPanel key={i} className="p-6 space-y-6">
              <div className="flex items-center gap-4">
                <Skeleton className="h-12 w-12 rounded-xl bg-[var(--muted)]" />
                <div className="space-y-2">
                  <Skeleton className="h-5 w-32 bg-[var(--muted)]" />
                  <Skeleton className="h-4 w-40 bg-[var(--muted)]" />
                </div>
              </div>
              <div className="space-y-4">
                <Skeleton className="h-12 w-full bg-[var(--muted)] rounded-lg" />
                <Skeleton className="h-12 w-full bg-[var(--muted)] rounded-lg" />
              </div>
            </GlassPanel>
          ))}
        ...
        <FadeIn>
          ...
          <TiltCard>
            <GlassPanel className="border-red-500/30 p-12 text-center">
              ...
              <p className="text-foreground font-semibold mt-6 text-xl">Unable to load settings</p>
              <p className="text-muted-foreground text-sm mt-2 max-w-sm mx-auto">{loadError}</p>
              <Button
                variant="outline"
        ...
        <div className="grid gap-6 md:grid-cols-2 mt-8">
          ...
          >
            <div className="p-4 bg-[var(--muted)] rounded-lg">
              <label className="text-sm font-medium text-foreground/80 mb-3 block">Color Theme</label>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between p-3 rounded-lg bg-[var(--muted)] border border-[var(--border)]">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-sm font-medium text-foreground/90">Sound Alerts</p>
              ...
              <div className="p-3 rounded-lg bg-[var(--muted)] border border-[var(--border)]">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-sm font-medium text-foreground/90">Timezone</p>
          ...
          >
            <Button
              variant="outline"
              className="w-full justify-start gap-4 h-auto py-4 bg-[var(--muted)] border-[var(--border)] text-foreground/80 hover:bg-[var(--muted)] hover:text-foreground"
              onClick={async () => {
                try {
                  const user = await jwtGetUser();
                  if (!user?.email) {
                    showFeedback('error', 'Unable to determine your email address.');
                    return;
                  }
                  await jwtSendPasswordResetEmail(user.email);
