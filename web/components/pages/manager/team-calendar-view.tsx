...
const LEAVE_TYPE_COLORS: Record<string, { gradient: string; text: string; dot: string }> = {
  ...
  LWP: { gradient: 'from-indigo-400 to-indigo-600', text: 'text-indigo-100', dot: 'bg-indigo-300' },
  CO: { gradient: 'from-teal-400 to-teal-600', text: 'text-teal-100', dot: 'bg-teal-300' },
};
const DEFAULT_LEAVE_COLOR = { gradient: 'from-violet-400 to-fuchsia-500', text: 'text-violet-50', dot: 'bg-violet-200' };
const MEMBER_COLOR_DOTS = [
  'bg-blue-500', 'bg-emerald-500', 'bg-violet-500', 'bg-amber-500', 'bg-red-500', 'bg-teal-500',
  'bg-pink-500', 'bg-indigo-500', 'bg-orange-500', 'bg-cyan-500', 'bg-lime-500', 'bg-rose-600',
];
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="h-8 bg-[var(--bg-surface-hover)] rounded-md w-48 animate-pulse" />
        <div className="flex items-center gap-2">
          <div className="h-9 w-9 bg-[var(--bg-surface-hover)] rounded-lg animate-pulse" />
          <div className="h-6 w-36 bg-[var(--bg-surface-hover)] rounded-md animate-pulse" />
          <div className="h-9 w-9 bg-[var(--bg-surface-hover)] rounded-lg animate-pulse" />
        </div>
      </div>
        ...
        <div className="lg:col-span-3">
          <GlassPanel className="p-4">
            <div className="grid grid-cols-7 gap-2 mb-2">
              {Array.from({ length: 7 }).map((_, i) => <div key={i} className="h-8 bg-[var(--bg-surface-hover)] rounded animate-pulse" />)}
            </div>
            <div className="grid grid-cols-7 gap-2">
              {Array.from({ length: 35 }).map((_, i) => <div key={i} className="h-[100px] bg-[var(--bg-surface-hover)] rounded-lg animate-pulse" />)}
            </div>
          </GlassPanel>
        </div>
...
export default function TeamCalendarView() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!leavesRes.ok) {
        throw new Error(`Failed to load team leave data (${leavesRes.status})`);
      }
  ...
  const { leavesByDate, holidaysByDate } = useMemo(() => {
    ...
    leaves.forEach(leave => {
      const current = parseDate(leave.start_date);
          ...
          action={
            <div className="flex items-center gap-2">
              <GlassPanel className="p-1 flex items-center rounded-xl">
                <Button variant="ghost" size="sm" onClick={() => setViewMode('grid')} className={viewMode === 'grid' ? 'bg-primary/15 text-primary' : ''}><Grid3X3 className="w-4 h-4" /></Button>
                <Button variant="ghost" size="sm" onClick={() => setViewMode('list')} className={viewMode === 'list' ? 'bg-primary/15 text-primary' : ''}><List className="w-4 h-4" /></Button>
              </GlassPanel>
              ...
              {teamMembersThisMonth.length > 0 ? (
                teamMembersThisMonth.map((member, index) => (
                  <div key={member.firstName + member.lastName} className="flex items-center gap-3 py-1.5">
                    <div className={`w-2 h-2 rounded-full ${MEMBER_COLOR_DOTS[index % MEMBER_COLOR_DOTS.length]}`} />
                    <span className="text-sm text-muted-foreground truncate">{getShortName(member.firstName, member.lastName)}</span>
                    <span className="ml-auto text-xs text-muted-foreground">{member.leaveCount}d</span>
                  </div>
                ))
              ) : (
                <p className="text-sm text-muted-foreground text-center py-4">No team leaves this month.</p>
              )}
            </SummaryCard>
          </div>
        </div>
  ...
  return (
    <GlassPanel interactive className="p-2 sm:p-4">
      <div className="grid grid-cols-7 gap-1 mb-2">
        {WEEKDAYS.map(day => <div key={day} className="text-center text-xs font-bold text-muted-foreground py-2 uppercase">{day}</div>)}
      </div>
  ...
  return (
    <div className={`
      h-[110px] rounded-lg border transition-colors duration-200 p-1.5 flex flex-col
        ...
        ${isToday ? 'ring-2 ring-primary/35 ring-offset-2 ring-offset-[var(--bg-surface-hover)]' : ''}
        ${isCurrentMonth && hasEvents ? 'bg-[var(--bg-surface-hover)]' : 'bg-[var(--accent)]'}
      ${!isCurrentMonth ? 'opacity-40' : ''}
    `}>
      <span className={`
        text-sm font-semibold
        ${isToday ? 'text-primary' : isCurrentMonth ? (isWeekend ? 'text-muted-foreground' : 'text-foreground/80') : 'text-muted-foreground'}
      `}>{day}</span>
      <div className="mt-1 space-y-0.5 overflow-y-auto scrollbar-thin scrollbar-thumb-[var(--muted)] scrollbar-track-transparent">
        {holidays.map(h => <HolidayPill key={h.id} holiday={h} />)}
        {leaves.slice(0, 3).map(l => <LeavePill key={l.id} leave={l} />)}
        {leaves.length > 3 && <div className="text-xs text-muted-foreground text-center mt-1">+{leaves.length - 3} more</div>}
      </div>
    </div>
  );
}
...
function LeavePill({ leave }: { leave: LeaveRequest }) {
  ...
  return (
    <div
      className={`w-full px-1.5 py-0.5 rounded text-[10px] font-medium truncate bg-gradient-to-br ${color.gradient} ${color.text} ${isPending ? 'opacity-60 border border-dashed border-[var(--border)]' : ''}`}
  ...
  return (
    <GlassPanel interactive>
      <div className="p-6 border-b border-[var(--border)]">
        <h3 className="text-lg font-semibold text-foreground">Leave & Holiday List</h3>
      </div>
      {eventDays.length === 0 ? (
        ...
        <div className="max-h-[600px] overflow-y-auto scrollbar-thin scrollbar-thumb-[var(--muted)] scrollbar-track-transparent">
          {eventDays.map(dayData => (
            <div key={formatDateKey(dayData.date)} className={`px-4 py-3 border-b border-[var(--border)] ${dayData.isToday ? 'bg-primary/10' : ''}`}>
              <div className="flex items-center gap-3 mb-2">
                <span className={`font-bold ${dayData.isToday ? 'text-primary' : 'text-foreground'}`}>
              ...
              {dayData.leaves.map(l => {
                ...
                return (
                  <div key={l.id} className={`flex items-center gap-3 p-2 rounded-md mt-1 bg-[var(--bg-surface-hover)] ${l.status === 'pending' ? 'opacity-70' : ''}`}>
                    <div className={`w-2 h-2 rounded-full ${color.dot}`} />
                    <span className="text-sm font-medium text-muted-foreground">{l.employee.first_name} {l.employee.last_name}</span>
...
function StatItem({ label, value, className = 'text-foreground' }: { label: string; value: string | number; className?: string }) {
  return (
