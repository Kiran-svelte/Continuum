...
type ActiveTab = 'team' | 'regularizations';
    ...
    <div className="p-4 sm:p-6 pb-32">
      <div className="space-y-8">
        ...
        <div className="space-y-2">
            <Skeleton className="h-8 w-64 bg-[var(--accent)]" />
            <Skeleton className="h-4 w-80 bg-[var(--accent)]" />
        </div>

        {/* Date navigation skeleton */}
        <GlassPanel className="p-4 flex justify-between items-center">
            <Skeleton className="h-8 w-8 bg-[var(--accent)] rounded-lg" />
            <Skeleton className="h-6 w-48 bg-[var(--accent)]" />
            <Skeleton className="h-8 w-8 bg-[var(--accent)] rounded-lg" />
        </GlassPanel>

        {/* Tabs skeleton */}
        <div className="flex gap-2">
            <Skeleton className="h-10 w-32 bg-[var(--accent)] rounded-lg" />
            <Skeleton className="h-10 w-48 bg-[var(--accent)] rounded-lg" />
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {[...Array(4)].map((_, i) => (
            <GlassPanel key={i} className="p-6 space-y-3">
                <Skeleton className="h-5 w-24 bg-[var(--accent)]" />
                <Skeleton className="h-8 w-16 bg-[var(--accent)]" />
            </GlassPanel>
          ))}
        </div>
        ...
        <GlassPanel className="p-6 space-y-4">
          <Skeleton className="h-6 w-40 bg-[var(--muted)]" />
                        <Skeleton className="h-6 w-40 bg-[var(--accent)]" />
          <div className="space-y-3">
            {[...Array(5)].map((_, i) => (
              <div key={i} className="flex justify-between items-center p-3 bg-[var(--muted)] rounded-lg">
                ...
                <Skeleton className="h-8 w-32 bg-[var(--muted)] rounded-lg" />
              </div>
            ))}
          </div>
        </GlassPanel>
        ...
        <FadeIn>
          ...
          <TiltCard>
            <GlassPanel className="mt-8 p-12 text-center border-red-500/30">
              ...
              <p className="text-foreground font-semibold mt-6 text-xl">Unable to load attendance</p>
              <p className="text-muted-foreground text-sm mt-2 max-w-sm mx-auto">{error}</p>
              <Button
                variant="outline"
                className="mt-6 bg-[var(--accent)] border-[var(--border)] text-foreground/80 hover:bg-[var(--muted)] hover:text-foreground"
  ...
  return (
    <div className="p-4 sm:p-6 pb-32">
      <StaggerContainer>
        ...
        <FadeIn>
          <GlassPanel className="mt-8 p-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <h3 className="text-lg font-bold text-foreground text-center">{formatDisplayDate(selectedDate)}</h3>
              {!isTodaySelected && (
        ...
        {/* ---- Tab navigation ---- */}
        <FadeIn>
          <div className="mt-6 flex gap-2 p-1.5 rounded-xl bg-[var(--muted)] w-fit border border-[var(--border)]">
            <TabButton id="team" activeTab={activeTab} setActiveTab={setActiveTab}>
              Team Status
            </TabButton>
            <TabButton id="regularizations" activeTab={activeTab} setActiveTab={setActiveTab}>
              <FileEdit className="w-5 h-5" />
              Regularizations
              {regularizations.length > 0 && (
                <span className="ml-2 px-2 py-0.5 text-xs font-bold rounded-full bg-amber-300 text-amber-950">
                  ...
                  <FadeIn>
                    <TiltCard>
                      <GlassPanel className="p-12 text-center">
                        <UserSearch className="w-12 h-12 mx-auto text-muted-foreground mb-4" />
                        <h3 className="text-xl font-semibold text-foreground">No Direct Reports</h3>
                        <p className="text-muted-foreground mt-2">Team members will appear here once assigned.</p>
                      </GlassPanel>
                    </TiltCard>
                  </FadeIn>
                ) : (
...
function TabButton({ id, activeTab, setActiveTab, children }: { id: ActiveTab, activeTab: ActiveTab, setActiveTab: (id: ActiveTab) => void, children: React.ReactNode }) {
...
function TeamMemberRow({ member }: { member: TeamMember }) {
    ...
    checked_in: {
      ...
      badge: (
        <div className="flex items-center gap-2 flex-wrap justify-end">
          ...
          {formatHours(member.totalHours) && <span className="text-xs text-muted-foreground">{formatHours(member.totalHours)}</span>}
        </div>
      ),
    },
    on_leave: {
  ...
  return (
    <motion.div
      ...
      className="bg-[var(--muted)] border border-[var(--border)] rounded-2xl p-3 flex items-center justify-between gap-4"
    >
      <div className="flex items-center gap-4 min-w-0">
        <div className={`w-12 h-12 rounded-full flex items-center justify-center text-lg font-bold text-foreground shrink-0 shadow-lg bg-gradient-to-br ${config.gradient}`}>
          {member.initials}
        </div>
        <div className="min-w-0">
          <p className="font-semibold text-foreground truncate">{member.name}</p>
          <p className="text-xs text-muted-foreground truncate">{member.designation || 'N/A'}</p>
        </div>
      </div>
...
function RegularizationPanel({ requests, loading, onAction, processingId, actionMsg }: {
  ...
  return (
    <FadeIn>
      <GlassPanel className="p-4 sm:p-6">
        <h3 className="text-xl font-bold text-foreground mb-4">Pending Regularization Requests</h3>
        ...
        {loading ? (
          <div className="space-y-3">
            {[1, 2, 3].map((i) => <Skeleton key={i} className="h-24 w-full bg-[var(--muted)] rounded-lg" />)}
          </div>
        ) : requests.length === 0 ? (
          <div className="py-16 text-center">
            ...
            <p className="text-foreground font-semibold mt-5 text-lg">All Caught Up</p>
            <p className="text-muted-foreground text-sm mt-2">No pending regularization requests from your team.</p>
          </div>
        ) : (
...
function RegularizationRequestCard({ request, onAction, processingId }: {
  ...
  return (
    <motion.div
      variants={itemVariants}
      className="bg-[var(--muted)] border border-[var(--border)] rounded-2xl p-4 flex flex-col sm:flex-row items-start gap-4"
    >
      <div className="flex items-center gap-4 flex-1 min-w-0">
        <div className="w-12 h-12 rounded-full bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center text-lg font-bold text-foreground shrink-0 shadow-lg shadow-purple-500/20">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-muted-foreground truncate">{request.employee.designation || 'N/A'}</p>
      ...
      <div className="flex-1 w-full sm:w-auto">
        ...
        <p className="text-sm text-foreground/90 line-clamp-2">{request.reason}</p>
      </div>
