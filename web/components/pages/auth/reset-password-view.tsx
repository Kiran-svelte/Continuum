...
import { jwtUpdatePassword } from '@/lib/auth-client';
...
function ResetPasswordContent() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      await jwtUpdatePassword(resetToken, password);
      setSuccess(true);
        ...
        <FadeIn className="w-full max-w-md z-20" direction="up">
          <TiltCard>
            <GlassPanel className="p-8 text-center">
              ...
              <h2 className="text-lg font-semibold text-foreground text-foreground">Invalid or expired link</h2>
              <p className="text-sm text-muted-foreground text-muted-foreground mt-2">
    ...
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <FadeIn className="w-full max-w-md z-20" direction="up">
        ...
        <div className="text-center mb-8 relative z-10">
          ...
          <h1 className="text-2xl font-bold text-foreground text-foreground">Set a new password</h1>
          <p className="text-muted-foreground text-muted-foreground mt-1">Choose a strong password for your account</p>
        </div>
          ...
          <GlassPanel className="p-8">
            {success ? (
              <div className="text-center py-4">
                ...
                <h2 className="text-lg font-semibold text-foreground text-foreground">Password updated!</h2>
                <p className="text-sm text-muted-foreground text-muted-foreground mt-2">Redirecting you to sign in...</p>
                <div className="mt-4">
                  <div className="w-8 h-8 border-2 border-primary border-t-transparent rounded-full animate-spin mx-auto" />
                </div>
              </div>
              ...
              <>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div className="space-y-1.5">
                    <label htmlFor="password" className="block text-sm font-medium text-foreground/90 text-foreground/90">
                      New Password
                    </label>
                    <Input
                      id="password"
                      ...
                      className="w-full h-11 px-4 bg-card border border-border text-foreground rounded-xl text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary/50 focus:shadow-[0_0_15px_rgba(var(--primary-rgb),0.15)] transition-all"
                      placeholder="••••••••"
                      required
                      minLength={8}
                  ...
                  {/* Confirm Password */}
                  <div className="space-y-1.5">
                    <label htmlFor="confirm" className="block text-sm font-medium text-foreground/90 text-foreground/90">
                      Confirm Password
                    </label>
                    <Input
                      id="confirm"
            ...
            <div className="relative my-6">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-4 text-muted-foreground bg-background backdrop-blur-sm rounded-full text-xs">or</span>
              </div>
            </div>

            {/* Back to sign in */}
            <p className="text-center text-sm text-muted-foreground text-muted-foreground">
