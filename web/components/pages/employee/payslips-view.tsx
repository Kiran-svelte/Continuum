    ...
    <>
        ...
        action={
          <TiltCard>
            <Select
              ...
              className="px-4 py-2.5 rounded-xl border border-[var(--border)] bg-[var(--bg-surface-hover)] text-sm font-semibold text-foreground shadow-[0_0_15px_rgba(255,255,255,0.1)] backdrop-blur-md focus:outline-none focus:ring-2 focus:ring-primary transition-all duration-300 hover:bg-[var(--accent)]"
              value={selectedYear}
              onChange={(e) => setSelectedYear(parseInt(e.target.value))}
            >
              {[0, -1, -2].map((offset) => {
                const y = new Date().getFullYear() + offset;
                return <option key={y} value={y} className="bg-[var(--bg-surface-hover)] text-foreground">{y}</option>;
              })}
            </Select>
          </TiltCard>
        }
      ...
      {!loading && !error && (
        <>
                    ...
                    <div className="p-6 pt-5 pb-5 relative z-10">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground mt-1 font-medium">{slips.length} months</p>
                        </div>
                    ...
                    <div className="p-6 pt-5 pb-5 relative z-10">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground mt-1 font-medium">PF + ESI + TDS + PT</p>
                        </div>
                    ...
                    <div className="p-6 pt-5 pb-5 relative z-10">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground mt-1 font-medium">Year to date</p>
                        </div>
          ...
          <FadeIn>
            <TiltCard>
              <GlassPanel className="glass-panel border-[var(--border)] shadow-2xl relative overflow-hidden">
                <div className="p-6 border-b border-[var(--border)] bg-[var(--bg-surface-hover)]">
                  <div className="flex items-center justify-between">
                    <h3 className="text-lg font-semibold text-foreground drop-shadow-md">Payslips - {selectedYear}</h3>
                ...
                <div className="p-0 backdrop-blur-md bg-[var(--accent)]">
                  {slips.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-16 text-center px-4">
                      <div className="flex h-14 w-14 items-center justify-center rounded-full bg-[var(--bg-surface-hover)] mb-4 shadow-[0_0_15px_rgba(255,255,255,0.1)]">
                        <FileText className="h-7 w-7 text-muted-foreground" />
                      </div>
                      <h3 className="text-base font-bold text-foreground drop-shadow-md">No payslips available</h3>
                      <p className="mt-1 text-sm text-muted-foreground max-w-xs font-medium">
                    ...
                    <div className="divide-y divide-[var(--border)]">
                      {slips.map((slip) => (
                        <div key={slip.id} className="flex items-center justify-between px-4 py-4 hover:bg-[var(--accent)] transition-colors group cursor-pointer">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <p className="text-sm font-bold text-foreground group-hover:text-primary transition-colors">{MONTHS[slip.month - 1]} {slip.year}</p>
                          ...
                          <div className="flex items-center gap-5">
                            <div className="text-right hidden sm:block">
                              <p className="text-base font-bold text-foreground group-hover:text-green-400 transition-colors drop-shadow-md">{formatCurrency(slip.net_pay)}</p>
      ...
      {selectedSlip && (
        <Modal isOpen={!!selectedSlip} onClose={() => setSelectedSlip(null)} title={`Payslip - ${MONTHS[selectedSlip.month - 1]} ${selectedSlip.year}`}>
          <div className="space-y-4">
            ...
            {/* Earnings */}
            <div>
              <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-2 drop-shadow-md">Earnings</h4>
              <div className="border border-[var(--border)] rounded-xl overflow-hidden glass-panel">
                {[
                  ...
                  <div key={row.label} className="flex justify-between px-4 py-2 text-sm border-b border-[var(--border)] last:border-0 hover:bg-[var(--bg-surface-hover)] transition-colors">
            ...
            {/* Deductions */}
            <div>
              <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-2 drop-shadow-md">Deductions</h4>
              <div className="border border-[var(--border)] rounded-xl overflow-hidden glass-panel">
                {[
            ...
            <div className="text-xs text-muted-foreground border border-[var(--border)] rounded-xl p-3 space-y-1 bg-[var(--accent)] backdrop-blur-sm">
              <p className="font-bold text-muted-foreground">Employer Contributions (not deducted from salary):</p>
