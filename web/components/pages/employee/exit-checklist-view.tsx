...
const CATEGORY_COLORS: Record<string, string> = {
  'IT & Access': 'bg-blue-100 text-blue-700',
  'Finance': 'bg-green-100 text-green-700',
  'HR & Admin': 'bg-purple-100 text-purple-700',
  'Knowledge Transfer': 'bg-amber-100 text-amber-700',
  'Assets': 'bg-rose-100 text-rose-700',
  'Compliance': 'bg-indigo-100 text-indigo-700',
  'Other': 'bg-[var(--accent)] text-muted-foreground',
};
    ...
    <div className="p-4 sm:p-8 pb-32 max-w-4xl mx-auto">
      ...
      <div className="space-y-8">
        ...
        <div className="space-y-2">
          <Skeleton className="h-8 w-48 bg-[var(--accent)]" />
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="glass-panel rounded-2xl border border-[var(--border)] p-6 space-y-2">
              <Skeleton className="h-4 w-20 bg-[var(--accent)]" />
              <Skeleton className="h-8 w-12 bg-[var(--accent)]" />
            </div>
          ))}
        </div>

        {/* Progress bar skeleton */}
        <div className="glass-panel rounded-2xl border border-[var(--border)] p-6 space-y-3">
          <div className="flex justify-between">
            <Skeleton className="h-4 w-24 bg-[var(--accent)]" />
            <Skeleton className="h-4 w-12 bg-[var(--accent)]" />
          </div>
          <Skeleton className="h-3 w-full bg-[var(--accent)]" />
        </div>

        {/* Checklist items skeleton */}
        <div className="space-y-6">
          {[...Array(2)].map((_, i) => (
            <div key={i} className="glass-panel rounded-2xl border border-[var(--border)] p-6">
              <div className="flex justify-between items-center mb-4">
                <Skeleton className="h-6 w-24 bg-[var(--accent)]" />
                <Skeleton className="h-4 w-20 bg-[var(--accent)]" />
              </div>
              <div className="space-y-3">
                {[...Array(3)].map((_, j) => (
                  <div key={j} className="flex items-start gap-4 p-4 rounded-xl bg-[var(--accent)]">
                    <Skeleton className="w-6 h-6 rounded-full bg-[var(--accent)] mt-1" />
                    <div className="flex-1 space-y-2">
                      <Skeleton className="h-4 w-3/4 bg-[var(--accent)]" />
                      <div className="flex gap-2">
                        <Skeleton className="h-5 w-20 rounded-full bg-[var(--accent)]" />
                        <Skeleton className="h-5 w-28 rounded-full bg-[var(--accent)]" />
                      </div>
                    </div>
                  </div>
                ))}
...
export default function ExitChecklistView() {
        ...
        {!loading && !error && checklists.length === 0 && (
          <FadeIn>
            <GlassPanel interactive className="text-center py-20">
              ...
              <h3 className="text-xl font-semibold text-foreground mb-2">No exit tasks assigned</h3>
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              ...
              <FadeIn>
                <GlassPanel interactive className="p-4">
                  ...
                  <p className={`text-3xl font-bold ${overdueCount > 0 ? 'text-red-400' : 'text-foreground'}`}>{overdueCount}</p>
                </GlassPanel>
              </FadeIn>
            </div>
            ...
            <FadeIn>
              <GlassPanel interactive className="p-5">
                ...
                <div className="w-full bg-[var(--accent)] rounded-full h-2.5 shadow-inner">
                  <motion.div
                ...
                <FadeIn key={cl.id}>
                  <GlassPanel interactive className="p-4 sm:p-6">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
                      ...
                      <span className="text-xs text-muted-foreground mt-2 sm:mt-0">{clCompleted}/{items.length} done</span>
                    </div>
                      ...
                      {items.map((item, idx) => {
                        ...
                        return (
                          <FadeIn key={`${cl.id}-${idx}`}>
                            <GlassPanel
                              className={`p-3 sm:p-4 flex items-start gap-4 transition-colors duration-300 hover:bg-[var(--bg-surface-hover)] ${
                                item.completed
                                  ? 'border-green-500/20'
                                  : isOverdue
                                  ? 'border-red-500/30'
