...
/** Shape of the 422 response when pending regularizations exist. */
interface PendingRegularizationWarning {
  pending_count: number;
  affected_employees: Array<{ emp_id: string; name: string; date: string }>;
}
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          <div>
            ...
            <p className="font-medium text-foreground">{slip.department || '--'}</p>
          </div>
          <div>
            <p className="text-muted-foreground">Period</p>
            <p className="font-medium text-foreground">{MONTHS[slip.month - 1]} {slip.year}</p>
          </div>
          <div>
            <p className="text-muted-foreground">Designation</p>
            <p className="font-medium text-foreground">{slip.designation || '--'}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-3">
          {[
            ...
            <div key={item.label} className="text-center p-2 rounded-lg bg-[var(--bg-surface-hover)]">
          ...
          {rows.map((row) => (
            <div
              ...
              className={`flex items-center justify-between px-4 py-2.5 text-sm ${
                row.type === 'total'
                  ? 'bg-blue-500/10 font-semibold border-t border-b border-[var(--border)]'
                  : row.type === 'total-deduction'
                  ? 'bg-red-500/10 font-semibold border-t border-b border-[var(--border)]'
                  : row.type === 'net'
                  ? 'bg-green-500/10 font-bold text-base border-t-2 border-[var(--border)]'
                  : row.type === 'info'
...
function SlipsTable({
  ...
  return (
    <div className="border-t border-[var(--border)]">
      <div className="flex items-center justify-between px-4 py-2 bg-[var(--bg-surface-hover)]">
      ...
      <div className="hidden md:block overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[var(--border)] bg-[var(--bg-surface-hover)] text-left">
          ...
          <tbody>
            {slips.map((s) => (
              <tr key={s.id} className="border-b border-[var(--border)] hover:bg-[var(--bg-surface-hover)] transition-colors">
                <td className="px-4 py-2.5">
                  <div>
                    <p className="font-medium text-foreground">{s.employee_name}</p>
                ...
                <td className="px-4 py-2.5 text-right font-bold text-foreground">{formatCurrency(s.net_pay)}</td>
          ...
          <tfoot>
            <tr className="bg-[var(--bg-surface-hover)] font-semibold">
              <td className="px-4 py-2 text-foreground" colSpan={2}>Total</td>
        ...
        {slips.map((s) => (
          <div key={s.id} className="p-4 space-y-2">
            <div className="flex items-center justify-between">
              <div>
                <p className="font-medium text-sm text-foreground">{s.employee_name}</p>
            ...
            <div className="grid grid-cols-3 gap-2 text-xs">
              ...
              <div>
                ...
                <p className="font-bold text-foreground">{formatCurrency(s.net_pay)}</p>
              </div>
            </div>
          </div>
        ))}
...
export default function PayrollView() {
  ...
  // Pending regularization warning (422 response from generate)
  const [regularizationWarning, setRegularizationWarning] = useState<PendingRegularizationWarning | null>(null);
  ...
  const fetchRuns = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/payroll/history?limit=50', { credentials: 'include', signal: controller.signal });
      if (res.ok) {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        await ensureMe();
        const empRes = await fetch('/api/employees?limit=1', { credentials: 'include', signal: controller.signal });
        if (empRes.ok) {
  ...
  async function handleGenerate(force = false) {
    if (employeeCount === 0) {
      toast.warning('No employees with salary structures', {
        description: 'Add at least one employee with a salary structure before generating payroll.',
        duration: 8000,
      });
      return;
    }
    ...
    setRegularizationWarning(null);
    setIsForceGenerating(false);
    try {
      const url = force ? '/api/payroll/generate?force=true' : '/api/payroll/generate';
      ...
      // Duplicate run — already exists for this period
      if (res.status === 409) {
        toast.error('Payroll already exists', {
          description: data.error || `A payroll run for ${currentMonthLabel} already exists. Scroll down to view it.`,
          duration: 8000,
        });
        return;
      }

      // Pending attendance regularizations — surface actionable warning
      if (res.status === 422 && data.pending_count) {
        setRegularizationWarning({
      ...
      if (!res.ok) {
        ...
        setGenerateResult({ success: false, message: errorMessage });
        toast.error('Payroll generation failed', { description: errorMessage });
        return;
      }
      ...
      toast.success(`Payroll generated for ${currentMonthLabel}`, {
        description: `${data.employee_count} employees • Net: ${new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 }).format(data.total_net)}`,
      });
      fetchRuns();
    } catch (err) {
  ...
  async function handleStatusTransition(runId: string, newStatus: string) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.error('Status update failed', { description: data.error || 'Could not update payroll status.' });
        return;
      }
  ...
  async function handleReject(runId: string) {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        toast.error('Rejection failed', { description: data.error || 'Could not reject payroll run.' });
        return;
      }
      setRejectRun(null);
      setRejectReason('');
  ...
  return (
    <StaggerContainer className="space-y-6">
      ...
      <PageHeader
        ...
        action={
          <div className="flex gap-2">
            ...
            <Button variant="primary" onClick={() => handleGenerate(false)} loading={generating} disabled={generating}>
              Generate Payroll
            </Button>
          </div>
        }
              ...
              <div className="p-4">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-foreground">{generateResult.success ? 'Payroll Generated' : 'Generation Failed'}</p>
                      ...
                      {generateResult.data && (
                        <div className="flex flex-wrap gap-3 mt-2">
                          ...
                          <span className="text-xs font-medium text-foreground">Gross: {formatCurrency(generateResult.data.total_gross)}</span>
                          <span className="text-xs font-medium text-foreground">Net: {formatCurrency(generateResult.data.total_net)}</span>
                        </div>
                      )}
                    </div>
                  </div>
                    <Button
                      ...
                      className="text-muted-foreground hover:text-foreground h-auto min-h-0 p-1"
                      aria-label="Dismiss"
                    >
                    <X className="h-4 w-4" />
                    </Button>
      ...
      {/* Pending regularization warning banner */}
      <AnimatePresence>
        {regularizationWarning && (
          <motion.div
            ...
            <GlassPanel className="border-amber-500/30 bg-amber-500/5">
              <div className="p-4 space-y-3">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-foreground">
                      {regularizationWarning.pending_count} pending attendance regularization{regularizationWarning.pending_count !== 1 ? 's' : ''}
                    </p>
                    <p className="text-sm text-muted-foreground mt-0.5">
                      These may affect LOP calculations. Resolve them first for accurate payroll, or proceed anyway.
                    </p>
                    {regularizationWarning.affected_employees.length > 0 && (
                      <div className="mt-2 flex flex-wrap gap-1.5">
                        {regularizationWarning.affected_employees.slice(0, 8).map((emp) => (
                          <span key={emp.emp_id} className="text-xs px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/20">
                            {emp.name} ({emp.date})
                          </span>
                        ))}
                        {regularizationWarning.affected_employees.length > 8 && (
                          <span className="text-xs text-muted-foreground">+{regularizationWarning.affected_employees.length - 8} more</span>
                        )}
                      </div>
                    )}
                  </div>
                ...
                <div className="flex gap-2 justify-end">
                  <Button variant="outline" size="sm" onClick={() => setRegularizationWarning(null)}>Dismiss</Button>
                  <Button
                    variant="primary"
                    size="sm"
                    onClick={() => handleGenerate(true)}
                    loading={generating && isForceGenerating}
                    disabled={generating}
                  >
                    Proceed Anyway
                  </Button>
            ...
            <div className="p-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold mt-1 text-foreground">{employeeCount}</p>
            ...
            <div className="p-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold mt-1 text-foreground">{getNextPayrollDate()}</p>
            ...
            <div className="p-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold mt-1 text-foreground">{currentMonthLabel}</p>
          ...
          <div>
            {runsLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center px-4">
                <div className="flex h-16 w-16 items-center justify-center rounded-full bg-[var(--bg-surface-hover)] mb-4">
                  <FileText className="h-8 w-8 text-muted-foreground" />
                </div>
                <h3 className="text-lg font-semibold text-foreground">No payroll runs yet</h3>
                <p className="mt-2 max-w-sm text-sm text-muted-foreground">
                  Click &quot;Generate Payroll&quot; to create the first run for {currentMonthLabel}.
                </p>
                <Button variant="primary" className="mt-6" onClick={() => handleGenerate(false)} loading={generating} disabled={generating}>
                ...
                {runs.map((run) => {
                  ...
                  return (
                    <div key={run.id} className="group">
                      {/* Run row */}
                      <div
                        className="flex flex-col md:flex-row md:items-center justify-between px-4 py-3 hover:bg-[var(--bg-surface-hover)] transition-colors cursor-pointer gap-3"
                      ...
                      >
                        <div className="flex items-center gap-4 min-w-0">
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-2 flex-wrap">
                              <p className="font-medium text-sm text-foreground">{MONTHS[run.month - 1]} {run.year}</p>
                        ...
                        <div className="flex items-center gap-4 md:gap-6 text-sm">
                          ...
                          <div className="text-right">
                            ...
                            <p className="font-bold text-foreground">{formatCurrency(run.total_net)}</p>
                          </div>
        ...
        <Modal isOpen={!!rejectRun} onClose={() => setRejectRun(null)} title="Reject Payroll Run">
          <div className="space-y-4">
            ...
            <div>
              ...
              <Textarea
                id="reject-reason"
                className="mt-1 w-full rounded-lg border border-[var(--border)] bg-[var(--bg-surface-hover)] px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30"
                rows={3}
                maxLength={500}
