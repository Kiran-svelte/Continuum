...
export default function ShiftsView() {
  ...
  function toggleSelectAllVisibleShifts() {
    const visibleIds = filteredShifts.map((shift) => shift.id);
    const allSelected = visibleIds.length > 0 && visibleIds.every((id) => selectedShiftIds.has(id));
  ...
  async function handleBulkDelete() {
    ...
    const confirmed = window.confirm(
      `Delete ${selectedShiftIds.size} selected shift${selectedShiftIds.size > 1 ? 's' : ''}? Assigned shifts cannot be deleted.`
    );
    if (!confirmed) return;
    ...
    try {
      ...
      if (failed.length > 0) {
        alert(`Deleted ${ids.length - failed.length}/${ids.length}. ${failed[0].data?.error || 'Some shifts failed to delete.'}`);
      }
          ...
          <div className="flex gap-2">
            <div className="relative">
              ...
              <Input
                ...
                className="pl-9 pr-4 py-2 rounded-lg border border-[var(--border)] bg-[var(--bg-surface-hover)] text-sm text-foreground w-56 placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30"
              />
            </div>
                      ...
                      <tr className="border-b border-[var(--border)] bg-[var(--bg-surface-hover)] text-left">
                        <th className="px-4 py-2.5 font-medium text-muted-foreground">
                          <input
                            ...
                            checked={filteredShifts.length > 0 && filteredShifts.every((shift) => selectedShiftIds.has(shift.id))}
                            onChange={toggleSelectAllVisibleShifts}
                            className="h-4 w-4"
                          />
                        </th>
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-4 py-2.5 text-center">
                              <span className="font-semibold text-foreground">{s.assigned_count}</span>
                            </td>
                            <td className="px-4 py-2.5 text-center">
                              {s.is_default ? (
                      ...
                      <div key={s.id} className="p-4 space-y-3">
                        ...
                        <div className="grid grid-cols-3 gap-2 text-xs">
                          <div>
                            ...
                            <p className="font-medium text-foreground">{fmtTime(s.start_time)}</p>
                          </div>
                          <div>
                            <p className="text-muted-foreground">End</p>
                            <p className="font-medium text-foreground">{fmtTime(s.end_time)}</p>
                          </div>
                          <div>
                            <p className="text-muted-foreground">Assigned</p>
                            <p className="font-semibold text-foreground">{s.assigned_count}</p>
                          </div>
                        </div>
      ...
      >
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Input
                ...
                className="mt-1 w-full rounded-lg border border-[var(--border)] bg-[var(--bg-surface-hover)] px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30"
              />
            </div>
            <div>
          ...
          {formStartTime && formEndTime && (
            <div className="bg-[var(--bg-surface-hover)] rounded-lg p-3 text-xs space-y-1">
              <div className="flex items-center gap-2">
        ...
        <div className="space-y-4">
              ...
              <div className="max-h-32 overflow-y-auto space-y-1">
                {assignShift.assigned_employees.map((ae) => (
                  <div
                    ...
                    className="flex items-center justify-between px-3 py-1.5 rounded-lg bg-[var(--bg-surface-hover)] text-xs"
                  >
                    <span className="font-medium text-foreground">{ae.employee_name}</span>
          ...
          <div>
            ...
            {empSearch && filteredEmployees.length > 0 && !assignEmployeeId && (
              <div className="mt-1 border border-[var(--border)] rounded-lg max-h-32 overflow-y-auto bg-[var(--bg-surface-hover)] backdrop-blur-xl">
      ...
      >
        <div className="text-center py-4">
          ...
          <p className="text-sm text-muted-foreground">
            Are you sure you want to delete <strong className="text-foreground">{deleteShift?.name}</strong>?
        ...
        <div className="flex gap-3 mt-4">
          <Button
            ...
            disabled={deleting}
            className="flex-1 px-4 py-2 rounded-lg border border-[var(--border)] text-foreground hover:bg-[var(--bg-surface-hover)] transition-colors disabled:opacity-50"
          >
            Cancel
          </Button>
          <Button
            ...
            disabled={deleting}
            className="flex-1 px-4 py-2 rounded-lg font-medium transition-colors disabled:opacity-50 bg-red-600 text-foreground hover:bg-red-700"
          >
