...
export default function SalaryComponentsView() {
  // Auth
  ...
  const SUMMARY_CARDS = [
    { label: 'Total Components', value: components.length, icon: Layers, color: 'text-blue-600', bg: 'bg-blue-50' },
    { label: 'Earnings', value: earningCount, icon: TrendingUp, color: 'text-emerald-600', bg: 'bg-emerald-50' },
    { label: 'Deductions', value: deductionCount, icon: TrendingDown, color: 'text-red-600', bg: 'bg-red-50' },
    { label: 'Active', value: activeCount, icon: ShieldCheck, color: 'text-violet-600', bg: 'bg-violet-50' },
  ];
  ...
  return (
    ...
    >
      ...
      {/* Tab Navigation */}
      <motion.div className="flex gap-1 bg-[var(--bg-surface-hover)]/50 rounded-lg p-1 w-fit" variants={itemVariants}>
        {TAB_ITEMS.map((tab) => {
          const Icon = tab.icon;
          return (
            <Button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-all ${
                activeTab === tab.key
                  ? 'bg-[var(--bg-surface-hover)] text-foreground shadow-sm'
        ...
        <motion.div variants={itemVariants}>
          <GlassPanel>
            <div className="p-6 border-b border-[var(--border)]">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-semibold text-foreground">All Components</h3>
                {components.length > 0 && (
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {components.map((comp) => (
                          <tr
                            key={comp.id}
                            className="border-b border-[var(--border)]/50 hover:bg-[var(--bg-surface-hover)]/30 transition-colors"
                          >
        ...
        <motion.div variants={itemVariants} className="space-y-4">
          ...
          <div className="relative w-full sm:w-64">
            ...
            <Input
              ...
              className="w-full pl-9 pr-4 py-2 rounded-lg border border-[var(--border)] bg-[var(--bg-surface-hover)] text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30"
            />
          </div>
