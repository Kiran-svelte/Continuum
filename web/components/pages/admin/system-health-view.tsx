      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="rounded-xl border border-[var(--border)] bg-[var(--bg-surface)] p-6 space-y-3">
        ...
        <div className="flex items-center gap-3">
          <Button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wider transition-all border ${autoRefresh
                ? 'border-[var(--status-success)]/30 bg-[var(--status-success)]/10 text-[var(--status-success)] shadow-[0_0_15px_rgba(52,211,153,0.1)]'
                : 'border-[var(--border)] bg-[var(--bg-surface)] text-muted-foreground'
              }`}
          >
            <div className={`w-2 h-2 rounded-full ${autoRefresh ? 'bg-[var(--status-success)] animate-pulse' : 'bg-[var(--accent)]'}`} />
        ...
        >
          <div className="p-8 relative z-10 flex items-center justify-between">
            <div className="flex items-center gap-6">
              <div className={`w-16 h-16 rounded-2xl bg-[var(--bg-surface)] flex items-center justify-center border border-[var(--border)] shadow-inner overflow-hidden relative`}>
                <div className="absolute inset-0 bg-gradient-to-br from-primary/10 to-transparent opacity-50" />
                {getStatusIcon(health?.status ?? 'unknown')}
              </div>
              <div>
                <p className="text-xl font-bold text-foreground tracking-tight">Main System Pipeline</p>
                <p className="text-sm text-muted-foreground mt-1 font-medium">
            ...
            <div className="text-right">
              ...
              <p className="text-[10px] text-muted-foreground uppercase tracking-[0.2em] mt-3 font-bold">Standard Operations</p>
            </div>
          </div>
        </GlowCard>
      </FadeIn>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <GlowCard color="rgba(59, 130, 246, 0.3)">
            <div className="p-6 relative z-10">
              <div className="flex items-center justify-between mb-4">
                <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">API Latency</p>
              ...
              <p className="text-[10px] text-muted-foreground mt-4 font-mono">NODE_GATEWAY_V2</p>
            </div>
          </GlowCard>
        </TiltCard>
          ...
          <GlowCard color="rgba(139, 92, 246, 0.3)">
            <div className="p-6 relative z-10">
              ...
              <div className="flex items-center gap-2">
                <div className={`w-2 h-2 rounded-full ${health?.checks?.database?.status === 'ok' ? 'bg-[var(--status-success)] shadow-[0_0_8px_rgba(52,211,153,0.8)]' : 'bg-[var(--status-danger)]'}`} />
              ...
              {health?.checks?.database?.latency && (
                <p className="text-[10px] text-[var(--status-success)]/60 mt-4 font-bold tracking-widest">{health.checks.database.latency}ms Jitter</p>
              )}
            </div>
          </GlowCard>
        </TiltCard>
        ...
        <TiltCard>
          <GlowCard color="rgba(16, 185, 129, 0.3)">
            <div className="p-6 relative z-10">
              <div className="flex items-center justify-between mb-4">
                <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Up-Time</p>
                <Clock className="w-5 h-5 text-emerald-500" />
              </div>
              <p className="text-2xl font-black text-foreground tracking-tighter line-clamp-1">
        ...
        <TiltCard>
          <GlowCard color="rgba(245, 158, 11, 0.3)">
            <div className="p-6 relative z-10">
              <div className="flex items-center justify-between mb-4">
                <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Memory</p>
                <MemoryStick className="w-5 h-5 text-amber-500" />
              </div>
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-black text-foreground tracking-widest">
                  {memoryUsedPercent ?? '0'}%
                </span>
              </div>
              <div className="mt-4 w-full h-1.5 bg-[var(--bg-surface)] rounded-full overflow-hidden border border-[var(--border)]">
                <motion.div
        ...
        <FadeIn>
          <GlassPanel className="h-full">
            <div className="p-6 border-b border-[var(--border)] flex items-center justify-between">
              ...
              <Badge variant="outline" className="text-[10px] border-[var(--border)]">v{health?.version || '1.0'}</Badge>
            </div>
            <div className="p-6 space-y-4">
              {health?.memory ? (
                Object.entries(health.memory).map(([key, value]) => (
                  <div key={key} className="flex items-center justify-between py-3 border-b border-[var(--border)] last:border-0 group">
                    <span className="text-sm font-medium text-muted-foreground group-hover:text-muted-foreground transition-colors capitalize">
                      {key.replace(/([A-Z])/g, ' $1')}
                    </span>
                    <span className="text-sm font-black text-foreground font-mono tracking-tighter">
                      {formatBytes(value as number)}
                    </span>
                  </div>
                ))
              ) : (
                <p className="text-center py-10 text-muted-foreground font-bold italic">No resource telemetry data</p>
              )}
            </div>
          </GlassPanel>
        </FadeIn>
          ...
          <GlassPanel className="h-full">
            ...
            <div className="p-6 space-y-4">
              {[
                ...
                { label: 'Active Checks', value: String(Object.keys(health?.checks || {}).length) },
                {
                ...
                <div key={item.label} className="flex items-center justify-between py-3 border-b border-[var(--border)] last:border-0 group">
                  <span className="text-sm font-medium text-muted-foreground group-hover:text-muted-foreground transition-colors">{item.label}</span>
                  <span className="text-sm font-bold text-foreground tracking-widest uppercase">{item.value}</span>
                </div>
              ))}
            </div>
          </GlassPanel>
      ...
      {healthHistory.length > 2 && (
        <FadeIn>
          <GlassPanel className="overflow-hidden">
            <div className="p-6 border-b border-[var(--border)] flex items-center justify-between bg-[var(--bg-surface)]">
