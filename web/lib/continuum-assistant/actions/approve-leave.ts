...
import { forwardAuthenticatedApi, logAssistantAction } from '@/lib/continuum-assistant/actions/http-execute';
import {
...
export async function executeApproveLeave(
  ...
  const { ok, data } = await forwardAuthenticatedApi(
    request,
    `/api/leaves/approve/${payload.request_id}`,
    {
      action,
      reason:
        action === 'reject'
          ? 'Rejected via Continuum Guide (user confirmed)'
  ...
  const finalStatus = typeof data.status === 'string' ? data.status : null;
  const isFinal = data.is_final === true || finalStatus === 'approved' || finalStatus === 'rejected';
