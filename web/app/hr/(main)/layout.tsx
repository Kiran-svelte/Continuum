...
export default function HRLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    fetch('/api/auth/me', { credentials: 'include' })
      ...
      .then(data => {
        if (!data) return;
        // Role verification: only hr, admin, director can access HR portal
        ...
        {(!isMobile || sidebarOpen) && (
          ...
          >
            ...
            <div className="px-6 py-4 border-b border-border dark:border-slate-800/50 flex items-center justify-between bg-gradient-to-r from-transparent to-transparent dark:from-slate-900 dark:to-slate-900/50">
              <div>
                <Link href="/hr/dashboard" onClick={closeSidebar}>
                  <h1 className="text-xl font-bold text-primary dark:text-emerald-400">{companyName}</h1>
                </Link>
            ...
            <div className="px-4 py-3 border-t border-border dark:border-slate-800/50 space-y-2 bg-gradient-to-r from-transparent to-transparent dark:from-slate-900/50 dark:to-slate-950">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 bg-green-100 dark:bg-emerald-500/20 rounded-full flex items-center justify-center text-sm font-medium text-green-600 dark:text-emerald-400">
                  👤
                </div>
