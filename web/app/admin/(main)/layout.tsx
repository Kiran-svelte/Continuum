...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    fetch('/api/auth/me', { credentials: 'include' })
      ...
      .then(data => {
        if (!data) return;
        // Middleware enforces admin-only — just load the portal for any authenticated user
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 h-14 bg-card/80 backdrop-blur-xl border-b border-border px-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <Link href="/admin/dashboard">
            <span className="text-lg font-bold bg-gradient-to-r from-indigo-500 to-violet-600 bg-clip-text text-transparent">{companyName}</span>
          </Link>
        </div>
        ...
        {(!isMobile || sidebarOpen) && (
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto">
              <SidebarNav items={ADMIN_NAV_ITEMS} onItemClick={closeSidebar} />
            </div>
            ...
            <div className="px-3 py-3 border-t border-border/50 dark:border-slate-800/40 space-y-2">
              ...
              <div className="flex items-center gap-3 px-2 py-2 rounded-lg hover:bg-muted/50 dark:hover:bg-slate-800/50 transition-colors">
                <div className="w-8 h-8 bg-gradient-to-br from-indigo-500/20 to-violet-500/5 dark:from-indigo-500/20 dark:to-violet-500/5 rounded-full flex items-center justify-center text-xs font-semibold text-indigo-600 dark:text-indigo-400">
                  {userName ? userName[0]?.toUpperCase() : 'A'}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-foreground truncate">{userName || 'My Account'}</p>
                  <p className="text-[11px] text-muted-foreground">System Admin</p>
                </div>
              </div>
              <PortalSwitcher />
              <SignOutButton />
