...
$databaseUrl = if ($env:NEON_DATABASE_URL) { $env:NEON_DATABASE_URL } else { $env:DATABASE_URL }
$directUrl = if ($env:NEON_DIRECT_URL) { $env:NEON_DIRECT_URL } else { $env:DIRECT_URL }

if ([string]::IsNullOrWhiteSpace($databaseUrl) -or [string]::IsNullOrWhiteSpace($directUrl)) {
    throw "Set NEON_DATABASE_URL/NEON_DIRECT_URL (or DATABASE_URL/DIRECT_URL) before running this script."
}
